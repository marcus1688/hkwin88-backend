...
//Staging
const rcb988Prefix = "ezwin9";
const rcb988Secret = process.env.HORSEBOOK_SECRET;
const webURL = "https://www.ezwin9.com/";
const rcb988APIURL = "https://wcaap.ac2boag.com";
...
const allGameCombinations = [
  //   { code: "SEXYBCRT", type: "LIVE" } rcb988 otherHorsebookModal,
...
router.post("/api/rcb988/launchGame", authenticateToken, async (req, res) => {
  try {
    //  gamelang === en or cn
    ...
    const createAccount = async (currency, config) => {
      const options = {
        method: "POST",
        url: `${rcb988APIURL}/wallet/createMember`,
        headers: { "content-type": "application/x-www-form-urlencoded" },
        data: querystring.stringify({
          cert: rcb988Secret,
          agentId: rcb988Prefix,
    ...
    for (const result of results) {
      ...
      if (result.status !== "0000" && result.status !== "1001") {
        console.log(
          `RCB988 error in registering ${result.currency} account:`,
          result.desc
        );
        return res.status(200).json({
          success: false,
          message: {
            en: "RCB988: Game launch failed. Please try again or contact customer service for assistance.",
            zh: "RCB988: 游戏启动失败，请重试或联系客服以获得帮助。",
            ms: "RCB988: Pelancaran permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
            zh_hk: "RCB988: 遊戲開唔到，老闆試多次或者搵客服幫手。",
            id: "RCB988: Peluncuran permainan gagal. Silakan coba lagi atau hubungi layanan pelanggan untuk bantuan.",
          },
        });
      }
    }
    ...
    if (launchResponse.data.status !== "0000") {
      console.log("RCB988 error in launching game", launchResponse.data);
      return res.status(200).json({
        success: false,
        message: {
          en: "RCB988: Game launch failed. Please try again or contact customer service for assistance.",
    ...
    console.log("RCB988 error in launching game", error.message);
    return res.status(200).json({
      success: false,
      message: {
        en: "RCB988: Game launch failed. Please try again or customer service for assistance.",
        zh: "RCB988: 游戏启动失败，请重试或联系客服以获得帮助。",
        ms: "RCB988: Pelancaran permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
        zh_hk: "RCB988: 遊戲開唔到，老闆試多次或者搵客服幫手。",
        id: "RCB988: Peluncuran permainan gagal. Silakan coba lagi atau hubungi layanan pelanggan untuk bantuan.",
...
router.post("/api/rcb988", async (req, res) => {
  try {
    ...
    switch (action) {
      ...
      case "bet": {
        ...
        if (currentUser.gameLock.rcb988.lock) {
          return res
            .status(200)
        ...
        const existingTransactions = await otherHorsebookModal
          .find(
            {
        ...
        if (totalBetAmount > 0) {
          ...
          if (newTransactions.length > 0) {
            await otherHorsebookModal.insertMany(
      ...
      case "cancelBet": {
        ...
        if (transactionsToUpdate.length > 0) {
          ...
          const [updatedUserBalance] = await Promise.all([
            ...
            otherHorsebookModal.bulkWrite(bulkOps),
          ]);
      ...
      case "adjustBet": {
        ...
        const [updatedUserBalance] = await Promise.all([
          ...
          otherHorsebookModal.findOneAndUpdate(
      ...
      case "refund": {
        ...
        const [updatedUserBalance] = await Promise.all([
          ...
          otherHorsebookModal.updateMany(
      ...
      case "settle": {
        ...
        for (const userId in transactionsByUser) {
          ...
          if (transactionsToUpdate.length > 0) {
            ...
            // Execute bulk update
            await otherHorsebookModal.bulkWrite(bulkOps);
      ...
      case "betNSettle": {
        ...
        const [currentUser, existingSettledTransaction] = await Promise.all([
          validateUser(userId, { "gameLock.rcb988.lock": 1 }),
          otherHorsebookModal
            .findOne(
              {
                roundId: platformTxId,
        ...
        await otherHorsebookModal.create({
          username: currentUser.gameId,
          roundId: platformTxId,
          bet: true,
          settle: true,
    ...
    console.error("RCB988 Error in calling ae96 API:", error.message);
...
router.post("/api/rcb988/getturnoverforrebate", async (req, res) => {
  try {
    const { date } = req.body;
    ...
    console.log("RCB988 QUERYING TIME", startDate, endDate);

    const records = await otherHorsebookModal.find({
      createdAt: {
        $gte: startDate,
        $lt: endDate,
      },
    ...
    console.log("RCB988: Failed to fetch win/loss report:", error.message);
    return res.status(500).json({
      error: "RCB988: Failed to fetch win/loss report",
    });
  }
});

router.get(
  "/admin/api/rcb988/:userId/dailygamedata",
...
router.get(
  "/admin/api/rcb988/:userId/gamedata",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      records.forEach((record) => {
        ...
        if (
          ...
          const horse = Object.fromEntries(gameCategories["Sports"]);

          if (horse["RCB988"]) {
            totalTurnover += horse["RCB988"].turnover || 0;
            totalWinLoss += horse["RCB988"].winloss || 0;
          }
        }
      });
...
router.get(
  "/admin/api/rcb988/dailykioskreport",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      console.error("RCB988: Failed to fetch win/loss report:", error);
      return res.status(500).json({
        success: false,
        error: "RCB988: Failed to fetch win/loss report",
      });
    ...
    try {
      ...
      records.forEach((record) => {
        ...
        if (
          ...
          if (horse["RCB988"]) {
            totalTurnover += Number(horse["RCB988"].turnover || 0);
            totalWinLoss += Number(horse["RCB988"].winloss || 0);
          }
        }
      });
