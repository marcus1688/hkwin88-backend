...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const jokerUsernames = [user.jokerGameName, user.jokerGameTwoName]
        .filter(Boolean)
        .map((u) => u.replace(/^FTXS\./, ""));
      ...
      const aggregationPipeline = filteredProviders.map(
        ({
          ...
          // Handle array of usernames or single username
          const usernameMatch = Array.isArray(usernameField)
            ? { username: { $in: usernameField } }
