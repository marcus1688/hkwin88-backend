...
const generateMonthlyReportImage = async (
  ...
  registerFont(path.join(process.cwd(), "Roboto-Bold.ttf"), {
    family: "Roboto",
  });
  ...
  try {
    const logo = await loadImage(
      "https://pub-92886b3c2cd44be98c01f0933462d4fd.r2.dev/hkwin88.png"
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      // 关闭 Tap & Go - 13/12/2025 23:59:00 UTC+8
      const closeDate = moment
        .tz("2025-12-13 23:59:00", "YYYY-MM-DD HH:mm:ss", "Asia/Kuala_Lumpur")
        .utc()
        .toDate();

      const closeBankLog = new BankTransactionLog({
        transactionId: "bank-closed-tapandgo-" + Date.now(),
        bankName: "Tap & Go",
        ownername: " ho Ying yeung",
        bankAccount: "123970493",
